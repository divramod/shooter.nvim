...
**The current shooter version is: `0.2.26`**

**Format:**
```
**`sho-<name>`** · **shooter v0.2.26** · model: **<model>**
```

- `<name>` — the command name (e.g., `prj-setup-infrastructure`, `gtd-epic-plan`, `cfg-make-healthy`)
- `shooter v0.2.26` — the shooter framework version (compiled into this file, do NOT read `.shooter/VERSION` for banners)
...
```markdown
## Banner
Output: **`sho-<command-name>`** · **shooter v0.2.26** · model: **<model>**
```
...
The registry enables `sho-sho-update-plugin` to batch-update all repos with latest rules.
...
**Custom types (configured by `sho-sho-setup-infrastructure`):** `theme`, `shot`
...
**Custom (configured by `sho-sho-setup-infrastructure`):** `discussed`, `planned`, `executing`, `verifying`
...
| `sho-h-show-progress` | Dashboard with progress bars and routing |
...
- Created by `sho-dev-release` skill, never manually
...
AI agents bump patch per-commit automatically. Human releases can be patch (default), minor, or major via `sho-dev-release`.

## Release Process

Human runs `/sho-dev-release` which:
...
`sho-sho-update-plugin` pulls the latest from GitHub, distributes to all CLIs, and verifies:
```
/sho-sho-update-plugin
```
...
- **Never create releases** — releases are human decisions via `sho-dev-release`
...
4. Do NOT run `sho-sho-setup-infrastructure` or initialize beads in the fork — keep forks clean for upstream PRs
...
- Do NOT run `sho-sho-setup-infrastructure` in forks — keep them clean for upstream contributions
...
**File location:** `ai/commands/sho-<namespace>-<name>.md` (flat, all files prefixed with `sho-`)

**Invocation:** `/sho-<namespace>-<name>` (uniform across all CLIs — no colon syntax)

**Namespaces (6):**

| Namespace | Purpose | Examples |
|-----------|---------|----------|
| `cfg` | Configuration | `sho-cfg-beads`, `sho-cfg-rules`, `sho-cfg-models` |
| `dev` | Development ops | `sho-dev-release`, `sho-dev-map-codebase`, `sho-dev-debug` |
| `gtd` | Task management | `sho-gtd-epic-plan`, `sho-gtd-issue-add`, `sho-gtd-quick-execute` |
| `self` | Shooter self-management | `sho-self-distribute-to-clis`, `sho-self-distribute-to-repos` |
| `sho` | Shooter framework ops | `sho-sho-init-project`, `sho-sho-setup-infrastructure`, `sho-sho-update-plugin` |
| `h` | Read-only display | `sho-h-show-help`, `sho-h-show-progress`, `sho-h-show-tour` |

**GTD entity pattern:** `sho-gtd-<entity>-<action>` where entity is `epic`, `issue`, `theme`, `shot`, `todo`, `quick`.

**YAML frontmatter `name:` field** must match the filename without extension (e.g., `sho-cfg-beads`).
...
|---------|------|-------------------|------------|
| `sho-gtd-epic-execute` | High | Multi-wave execution spans long context | Write manifest + checkpoint every wave |
| `sho-gtd-epic-plan` | Medium | Research phase generates large context | Write plan to beads notes before execution |
| `sho-gtd-verify-work` | Medium | Verification + gap analysis in one session | Checkpoint before/after each verification step |
| `sho-gtd-epic-discuss` | Low | Discussion fits in single context window | Write decisions to epic design field |
| `sho-gtd-issue-add` | None | Single atomic operation | No mitigation needed |
| `sho-gtd-resume-work` | None | Reads state, doesn't modify | Already compaction-safe by design |
| `sho-h-show-progress` | None | Read-only dashboard | No mitigation needed |
