...
1. **Plan First**: Create an epic bead (`bd create --type=epic`) and write the full plan to its `--notes` field. The epic bead is the **single source of truth** for the plan — it survives compaction and session crashes. Also write a human-readable copy to `.shooter/plans/<YYYY-MM-DD>_<HH-MM>_<short-description>.md`.
2. **Create Child Issues**: Break the plan into child beads (`bd create --type=task --parent=<epic-id>`) BEFORE starting work. Each child must have a complete execution prompt in its `--description`.
3. **Verify Plan**: Check in before starting implementation
4. **Track Progress**: Mark beads in_progress when starting, closed with `--reason` when done. Write EVENT checkpoints to the epic's comments (`bd comments add <epic-id> "EVENT: ..."`) at every state transition.
5. **Explain Changes**: High-level summary at each step
6. **Document Results**: Close the epic bead with a summary reason. Update `.shooter/plans/<plan>.md` with results.
7. **Capture Lessons**: Update `.shooter/context.md` after corrections

**Why beads over files:** Beads persist across compaction, sessions, and crashes. `.shooter/plans/` files are a human-readable backup. An agent waking up after compaction runs `bd show <epic-id>` and `bd children <epic-id>` to reconstruct the full plan and progress — no conversation memory needed.
...
**The current shooter version is: `0.2.31`**

**Format:**
```
**`sho-<name>`** · **shooter v0.2.31** · model: **<model>**
```

- `<name>` — the command name (e.g., `prj-setup-infrastructure`, `gtd-epic-plan`, `cfg-make-healthy`)
- `shooter v0.2.31` — the shooter framework version (compiled into this file, do NOT read `.shooter/VERSION` for banners)
...
```markdown
## Banner
Output: **`sho-<command-name>`** · **shooter v0.2.31** · model: **<model>**
```
...
```bash
bash ~/.config/shooter/_source/ai/scripts/shell/sho_util-log-command.sh "<command-name>" --model "<model>"
```

Where `~/.config/shooter/_source/ai` is the centralized shooter source directory for ALL CLIs. `<model>` is the model you are running on (e.g., `opus`, `sonnet`, `gemini-2.5-pro`). The CLI is auto-detected from the script's install path — no `--cli` flag needed. This:
...
The registry enables `aip-update-plugin` to batch-update all repos with latest rules.
...
6. **CRITICAL — Commit, push, sync in ONE call** using `sho_util-commit.sh`:
   ```bash
   bash /Users/mod/.config/shooter/_source/ai/scripts/shell/sho_util-commit.sh \
...
2. **Persist** the Q&A to `.shooter/q-and-a.md` — **prepend** (newest first) below the `# Q&A` heading. Format:
   ...
   **Theme:** <theme-slug>
   ```
   Include full timestamp so newest questions are always at the top and easy to spot. **Theme** is required — use the theme slug from the shot context (e.g., `ai`, `cli`, `nvim`). For cross-cutting questions, use `_project`.
...
2. **Enter plan mode** or run `aig-epic-plan` to break it into child issues
3. **Execute** via `aig-epic-execute` or manually work through child issues
...
**Custom types (configured by `aip-setup-infrastructure`):** `theme`, `shot`
...
**Custom (configured by `aip-setup-infrastructure`):** `discussed`, `planned`, `executing`, `verifying`
...
### Namespace: `error:` -- Error Tracking

Used to classify and track errors encountered during development. See Error Culture rule for full lifecycle.

- `error:silent-failure` -- operation failed but error was suppressed (e.g., `2>/dev/null || true`)
- `error:non-idempotent` -- script failed on re-run
- `error:missing-validation` -- pre-flight check not performed, led to cryptic error
- `error:regression` -- previously working code broke due to unrelated change
- `error:test-gap` -- no test coverage for this failure path
...
| Command | Purpose |
|---|---|
| `aic-beads` | One-time setup: creates themes + shots epics |
| `aig-theme-add` | Create a new theme with auto shots epic |
| `aig-theme-show` | Display theme with its epics |
| `aig-epic-add` | Create epic under a theme |
| `aig-epic-show` | Display epic with child issues |
| `aig-issue-add` | Create issue under an epic |
| `aig-issue-show` | Display issue details |
| `aig-epic-discuss` | Gray area discussion, write decisions to design field |
| `aig-epic-plan` | Research → plan → check loop, creates child issues |
| `aig-epic-execute` | Wave-based parallel execution of child issues |
| `aig-verify-work` | UAT + diagnosis + gap planning |
| `aig-resume-work` | Context restoration from beads state |
| `aih-show-progress` | Dashboard with progress bars and routing |
...
- **Tasks/Bugs/Features**: Include what to do and why. For planned issues (from `aig-epic-plan`), this is the execution prompt.
...
Every commit that changes user-facing behavior should include a changelog entry in the `[Unreleased]` section of CHANGELOG.md.

**Automatic handling:** `sho_util-commit.sh` auto-derives changelog entries for `feat`, `fix`, `refactor`, `perf`, and `docs` commits. No manual action needed for these types.

**Override:** Use `--changelog "custom entry text"` to provide a curated entry instead of the auto-derived one.

**Skip:** Use `--no-changelog` to explicitly skip the changelog entry (e.g., for internal-only refactors).

**Excluded types:** `chore`, `ci`, `style`, `test` commits do not generate changelog entries (internal-only changes).

**Entry quality guidelines:**
- Use imperative mood: "Add", "Fix", "Remove" (not "Added", "Fixed")
- Be specific: name the feature or area affected
- Be self-contained: understandable without reading the commit
...
**Per-commit version sync:**

Projects can configure automatic version file syncing in `.shooter/cfg/ai/config.yaml`:
...
When `version.files` is non-empty, `sho_util-commit.sh` automatically syncs the version from `.shooter/VERSION` to the listed files on every commit. Supported formats: `.toml` and `.json`.

When `version.files` is empty (default), version file sync only happens during releases via `sho_release-sync-versions.sh` (which auto-discovers version files).
...
- Created by `aid-release` skill, never manually
...
- When encountering errors in scripts or tools: immediately create a bug bead or fix directly (see Error Culture rule)
- Stage specific files, not `git add -A` or `git add .`
- After modifying any rule file in `ai/rules/`: run `bash ai/scripts/shell/sho_ensure-rules.sh` to regenerate AGENTS.md, CLAUDE.md, GEMINI.md
...
3. **Idempotent** — safe to run multiple times (see Idempotency Patterns below)
...
7. **Steps header** — every script must include a `# Steps:` section in its header listing each logical step (see Shell Script Conventions)

## Idempotency Patterns

Scripts MUST be idempotent — running them multiple times produces the same result as running once, with no harmful side effects on re-run.
...
# File creation — check existence first
if [[ ! -f "$file" ]]; then
    printf "content" > "$file"
    printf "created: %s\n" "$file"
else
...
**Check before append:**
```bash
# Prevent duplicate entries in files
...
Never suppress errors silently on critical operations. The pattern `2>/dev/null || true` is only acceptable for:
- Non-critical operations (logging, metrics, optional features)
- Explicit fallback logic with a comment explaining why

**Banned for:** git operations, file mutations, beads operations, version bumping.

```bash
# Bad — silent failure on critical operation
...
After mutating state, verify the mutation succeeded:
```bash
printf "%s" "$version" > "$version_file"
if [[ "$(cat "$version_file")" != "$version" ]]; then
    printf "error: version file write failed\n" >&2
    exit 2
fi
```
...
Scripts should minimize side effects beyond their stated purpose:

- **Do one thing well** — a version bump script bumps versions, not also reformatting files
- **Document all side effects** — if a script modifies files beyond its primary target, list them in the Steps header
- **No hidden network calls** — scripts should not make network requests unless their name/purpose implies it
- **Clean up temp files** — use `trap 'rm -f "$tmp_file"' EXIT` for temporary files
- **Predictable output** — same inputs should produce same outputs regardless of environment state

## Templates Over Hardcoding

When a script needs to create or initialize a file with boilerplate content, use a template from `ai/templates/` instead of hardcoding the content inline.

**Resolution order** (same as `sho_ensure-labels.sh`):
1. `~/.config/shooter/_source/ai/templates/<file>` (centralized source)
2. `$SCRIPT_DIR/../../templates/<file>` (relative to script location)
3. Minimal hardcoded fallback only if no template is found

**Why:**
- Templates are editable in one place — changes propagate to all scripts that use them
- Hardcoded strings inside scripts are invisible, hard to maintain, and easy to forget
- Templates are distributed to all CLIs automatically via `sho_dist-to-clis.sh`

**Pattern:**
```bash
local tmpl=""
if [[ -f "$HOME/.config/shooter/_source/ai/templates/<file>" ]]; then
    tmpl="$HOME/.config/shooter/_source/ai/templates/<file>"
elif [[ -f "$script_dir/../../templates/<file>" ]]; then
    tmpl="$(cd "$script_dir/../../templates" && pwd)/<file>"
fi
if [[ -n "$tmpl" ]]; then
    cp "$tmpl" "$target"
else
...
CHANGELOG.md maintains an `[Unreleased]` section at the top that accumulates entries between releases:

- **Per-commit:** `sho_util-commit.sh` appends entries to `[Unreleased]` for feat/fix/refactor/perf/docs commits
- **At release:** `sho_release-changelog.sh` converts `[Unreleased]` to a versioned `[X.Y.Z] - YYYY-MM-DD` section
- **Fallback:** If `[Unreleased]` is empty at release time, entries are generated from git history

Date format: `YYYY-MM-DD HH:MM` (date and time to the minute).

## Update Distribution

`aii-distribute-to-clis` distributes to ALL installed CLIs simultaneously:
...
`aip-update-plugin` pulls the latest from GitHub, distributes to all CLIs, and verifies:
```
/aip-update-plugin
```
...
4. Do NOT run `aip-setup-infrastructure` or initialize beads in the fork — keep forks clean for upstream PRs
...
- Use `aid-fork-repo` command when available
- Do NOT run `aip-setup-infrastructure` in forks — keep them clean for upstream contributions
...
**Format:** `ai<letter>-<name>` — short 3-letter prefix per namespace

**File location:** `ai/commands/<command-name>.md` — filename equals command name (no extra prefix)

**Invocation:** `/<command-name>` (uniform across all CLIs)

**Rule:** The YAML frontmatter `name:` field MUST match the filename without extension. The filename IS the command name.

**Namespace prefixes (6):**

| Prefix | Namespace | Purpose | Examples |
|--------|-----------|---------|----------|
| `aic-` | cfg | Configuration | `aic-beads`, `aic-rules`, `aic-models` |
| `aid-` | dev | Development ops | `aid-release`, `aid-map-codebase`, `aid-debug` |
| `aig-` | gtd | Task management | `aig-epic-plan`, `aig-issue-add`, `aig-quick-execute` |
| `aii-` | self | Shooter self-management | `aii-distribute-to-clis`, `aii-distribute-to-repos` |
| `aih-` | h | Read-only display | `aih-show-help`, `aih-show-progress`, `aih-show-tour` |
| `aip-` | sho | Shooter framework ops | `aip-init-project`, `aip-setup-infrastructure`, `aip-update-plugin` |

**GTD entity pattern:** `aig-<entity>-<action>` where entity is `epic`, `issue`, `theme`, `shot`, `todo`, `quick`.
...
| `health` | Health checks, diagnostics | `sho_health-check.sh`, `sho_health-sync-rules-yaml.sh` |
...
Agents are Task tool subagent types. Named by their functional role, not by namespace. Use `sho-` prefix (matching `sho-` command prefix).

| Pattern | Examples |
|---------|----------|
| `sho-<role>` | `sho-executor`, `sho-verifier`, `sho-debugger` |
| `sho-<entity>-<role>` | `sho-epic-planner`, `sho-epic-researcher` |
| `sho-<domain>-<role>` | `sho-codebase-mapper`, `sho-history-interpreter` |
...
Examples: `rules.yaml`, `config.yaml`, `labels.json`, `project-context.md`
...
AI agent context windows get compacted (summarized) during long sessions. Orchestrators are stateless after compaction -- all state must live in beads, not in conversation memory.

## Core Principle

Treat every command entry point as if the agent just woke up with no memory. The beads database is the single source of truth. Conversation context is ephemeral; beads are permanent. Design workflows so that compaction at any point leaves enough state in beads to resume without loss.

## State Reconstruction Pattern

On entry to any long-running command, reconstruct state from beads before acting:

```bash
# 1. Read epic status to understand where we are
bd epic status <epic-id>

# 2. Get all child issues with their statuses
bd children <epic-id>

# 3. Check what's in progress (may be stale from a crashed session)
bd list --status=in_progress --parent=<epic-id>

# 4. Check what's blocked
bd list --status=blocked --parent=<epic-id>

# 5. Read epic notes for the execution manifest
bd show <epic-id>

# 6. Read recent comments for checkpoint history
bd comments list <epic-id>
```

From this data, derive: which wave is current, which issues are done, which are in flight, and what to do next. Never rely on variables from earlier in the conversation.

## Checkpoint Pattern

Write structured checkpoints to epic comments at every state transition. Use the `EVENT:` prefix so checkpoints are machine-parseable:

```bash
# Wave lifecycle
bd comments add <epic-id> "EVENT: wave 1 started [sho-abc.1, sho-abc.2, sho-abc.3]"
bd comments add <epic-id> "EVENT: sho-abc.1 closed - implemented auth middleware"
bd comments add <epic-id> "EVENT: sho-abc.2 closed - added rate limiting"
bd comments add <epic-id> "EVENT: wave 1 complete (2 closed, 1 blocked)"

# Verification lifecycle
bd comments add <epic-id> "EVENT: verification started"
bd comments add <epic-id> "EVENT: verification PASS"
bd comments add <epic-id> "EVENT: verification FAIL"

# UAT lifecycle
bd comments add <epic-id> "EVENT: UAT sho-abc.1 accepted"
bd comments add <epic-id> "EVENT: UAT sho-abc.3 rejected"
```

**When to checkpoint:**
- Before starting a wave
- After closing each issue within a wave
- After completing a wave (with summary counts)
- Before and after verification
- After each UAT decision

## Execution Manifest Pattern

Before executing an epic, write the full wave plan to the epic's notes field. This survives compaction and gives any future agent the complete plan:

```bash
bd update <epic-id> --notes="$(cat <<'EOF'
Execution manifest:
- Wave 1: sho-abc.1 (auth middleware), sho-abc.2 (rate limiting), sho-abc.3 (logging)
- Wave 2: sho-abc.4 (integration tests) [depends on wave 1]
- Wave 3: sho-abc.5 (documentation) [depends on wave 2]
EOF
)"
```

The manifest answers "what is the plan?" without needing conversation history. After compaction, an agent reads the manifest and the checkpoint comments to know exactly where execution stands.

## Idempotency Rules

Every action must be safe to repeat. Check state before acting:

```bash
# Before closing an issue -- check it's not already closed
status=$(bd show <issue-id> --json | jq -r '.status')
if [ "$status" != "closed" ]; then
  bd close <issue-id> --reason="..."
fi

# Before starting a wave -- check which issues are already done
bd children <epic-id> --json | jq '.[] | select(.status == "open")'

# Before creating a checkpoint -- acceptable to duplicate (comments are append-only)
bd comments add <epic-id> "EVENT: wave 2 started [sho-abc.4]"
```

**Patterns:**
- **Check before close** -- never close an already-closed bead
- **Check before status update** -- don't transition backwards (closed -> in_progress)
- **Filter before wave start** -- skip issues already closed from a previous attempt
- **Comments are always safe** -- append-only, duplicates are harmless

## Compaction Preservation Hints

When building commands that will run in long sessions, include hints for the context summarizer. Place critical state in a format the summarizer is likely to preserve:

**What to keep visible:**
- The epic ID being executed (e.g., `Working on epic sho-abc`)
- The current wave number and its issue IDs
- The execution manifest (wave plan)
- The last checkpoint written

**How to surface it:** Repeat critical identifiers in your output. The summarizer retains information that appears multiple times or in structured formats:

```
Current state: epic sho-abc, wave 2, issues [sho-abc.4, sho-abc.5]
Last checkpoint: EVENT: wave 1 complete (3 closed, 0 blocked)
```

**What survives compaction regardless:**
- Beads database (external to context)
- Epic comments (checkpoints)
- Epic notes (execution manifest)
- File system artifacts (plans, context files)

## Command Risk Levels

| Command | Risk | Compaction Concern | Mitigation |
|---------|------|-------------------|------------|
| `aig-epic-execute` | High | Multi-wave execution spans long context | Write manifest + checkpoint every wave |
| `aig-epic-plan` | Medium | Research phase generates large context | Write plan to beads notes before execution |
| `aig-verify-work` | Medium | Verification + gap analysis in one session | Checkpoint before/after each verification step |
| `aig-epic-discuss` | Low | Discussion fits in single context window | Write decisions to epic design field |
| `aig-issue-add` | None | Single atomic operation | No mitigation needed |
| `aig-resume-work` | None | Reads state, doesn't modify | Already compaction-safe by design |
| `aih-show-progress` | None | Read-only dashboard | No mitigation needed |

**Rule of thumb:** Any command that takes more than 5 minutes or touches more than 3 beads needs checkpointing. Any command that spawns subagents needs an execution manifest.


# Error Culture

Errors are learning opportunities. Every error detected — in scripts, commands, agents, or workflows — triggers a structured response: track, analyze, fix, prevent.

## Philosophy

The shooter framework treats errors as first-class citizens:

1. **Errors are signals** — they reveal wrong assumptions about the system
2. **Errors are opportunities** — each fix makes the system more resilient
3. **Errors are shared knowledge** — tracking them helps future agents avoid the same pitfalls

Blameless by default. Focus on "what system change prevents this?" not "who caused this?"

## Error Lifecycle

Every error follows a five-step lifecycle:

### 1. Detection

Errors are detected through:
- Script failures (non-zero exit codes, `error:` prefixed output)
- Test failures (unit, integration, E2E)
- User reports (shots, verbal feedback)
- Agent observation (unexpected behavior during task execution)
- Pre-commit checks (format, lint, typecheck, build)

### 2. Tracking

When an error is detected, **immediately** create a tracking artifact:

**During active task work:**
```bash
# Create a bug bead for the error
bd create --type=bug --parent=<current-epic-or-shots-epic> \
  --title="Fix: <concise error description>" \
  --description="<what happened, what was expected, what was actual>" \
  --labels="type:issue:bug,theme:<slug>,error:<category>"
```

**Error categories** (use as labels):
- `error:silent-failure` — operation failed but error was suppressed
- `error:non-idempotent` — script failed on re-run
- `error:missing-validation` — pre-flight check not performed
- `error:regression` — previously working code broke
- `error:test-gap` — no test coverage for this failure path

**If the error is in the current task's critical path:** fix it immediately, then close the bead.
**If the error is not blocking:** create the bead and continue. The bead becomes backlog.

### 3. Analysis

Before fixing, understand the root cause:
- **What assumption was wrong?** (e.g., "assumed git add always succeeds")
- **What context was missing?** (e.g., "didn't know macOS sed differs from GNU sed")
- **Is this a pattern?** (e.g., "third time a CLI format conversion broke")

Write the analysis into the bead's description or notes.

### 4. Fix

Implement the fix with:
- A test that reproduces the error (when feasible)
- The minimal code change to resolve the root cause
- Verification that the fix works

### 5. Prevention

After fixing, prevent recurrence:
- **Update rules** if the error reveals a missing guideline
- **Update context.md** if the error reveals a project-specific pattern
- **Add to concerns.md** if the error is part of a larger systemic issue
- **Add a quality gate** if the error should be caught automatically

## Rules for AI Agents

### Encountering Errors During Development

When you encounter an error in any script, command, or workflow while working:

1. **Do not ignore it.** Even if it doesn't block your current task.
2. **Assess urgency:**
   - **Blocking your task?** Fix it now. Create a bead, fix, close with reason.
   - **Not blocking?** Create a bug bead with full context. Continue your task. The bead ensures the error is not lost.
3. **Never suppress errors to make things work.** If a script fails, fix the script — don't add `|| true` to work around it.
4. **Log the learning.** If you learned something from the error, add it to `.shooter/context.md` (AI Learnings section).

### Writing Error-Resilient Code

- **Fail fast, fail loud** — use `set -euo pipefail` and meaningful error messages
- **Check preconditions** — verify tools, files, and state exist before operating
- **Verify postconditions** — confirm mutations succeeded after performing them
- **Clean up on failure** — use `trap` to remove temp files on script exit
- **Log context** — error messages should include: what was attempted, what failed, and what to check

### Close Reason Format for Error Fixes

When closing a bead that fixed an error, use this format:

```
Fixed: <what was wrong>. Root cause: <why it happened>. Prevention: <what was added to prevent recurrence>. Files: <paths>.
```

## Integration with Concerns

`.shooter/codebase/_project/CONCERNS.md` is generated by the codebase mapper and tracks systemic issues. The error culture complements it:

- **Concerns** = systemic patterns discovered during code review (mapper output)
- **Error beads** = specific incidents discovered during development (runtime)

When an error bead reveals a systemic pattern, add it to concerns.md. When a concern from concerns.md causes a specific failure, create an error bead linking back to the concern.

## Error Budget

Each epic has an implicit error budget — if more than 20% of child issues involve error fixes or rework:

1. **Pause execution** — stop creating new issues
2. **Analyze** — what pattern is causing the failures?
3. **Write a checkpoint** — `bd comments add <epic-id> "EVENT: error budget exceeded, pausing for analysis"`
4. **Fix the root cause** — before resuming execution
5. **Update rules** — add prevention measures
