...
function M.get_all_shots(project_filter)
  ...
  for _, repo_path in ipairs(repos) do
    ...
    -- Find all .md files in .shooter/ai/shotfiles and subprojects
    local prompts_dir = repo_path .. '/.shooter/ai/shotfiles'
    ...
    if utils.dir_exists(projects_dir) then
      handle = io.popen('find "' .. projects_dir .. '" -path "*/.shooter/ai/shotfiles/*.md" -type f 2>/dev/null')
      if handle then
        for filepath in handle:lines() do
          local project = M.detect_project_from_path(filepath)
          if M.repo_matches_filter(repo_name, project_filter) then
