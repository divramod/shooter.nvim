...
-- Known folder names (subdirectories of .shooter/project-shotfiles/)
local FOLDER_NAMES = defaults.get_folder_names()

-- Detect which folder a file belongs to
-- Files in .shooter/project-shotfiles/<folder>/ belong to that folder
-- Files directly in .shooter/project-shotfiles/ belong to "prompts"
...
function M.detect_file_folder(filepath)
  ...
  for _, folder in ipairs(FOLDER_NAMES) do
    if folder ~= 'prompts' then
      local pattern = '/.shooter/project-shotfiles/' .. folder .. '/'
      if filepath:find(pattern, 1, true) then
        return folder
      end
    end
  end
  -- Files directly in .shooter/project-shotfiles/ are "prompts"
  if filepath:find('/.shooter/project-shotfiles/[^/]+%.md$') then
    return 'prompts'
  end
...
function M.detect_file_project(filepath, git_root)
  ...
  -- Check if file is in projects/<name>/.shooter/project-shotfiles/
