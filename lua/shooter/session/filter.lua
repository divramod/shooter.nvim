...
-- Known folder names (subdirectories of docs/shotfiles/)
local FOLDER_NAMES = defaults.get_folder_names()

-- Detect which folder a file belongs to
-- Files in docs/shotfiles/<folder>/ belong to that folder
-- Files directly in docs/shotfiles/ belong to "prompts"
...
function M.detect_file_folder(filepath)
  ...
  for _, folder in ipairs(FOLDER_NAMES) do
    if folder ~= 'prompts' then
      local pattern = '/docs/shotfiles/' .. folder .. '/'
      if filepath:find(pattern, 1, true) then
        return folder
      end
    end
  end
  -- Files directly in docs/shotfiles/ are "prompts"
  if filepath:find('/docs/shotfiles/[^/]+%.md$') then
    return 'prompts'
  end
...
function M.detect_file_project(filepath, git_root)
  ...
  -- Check if file is in projects/<name>/docs/shotfiles/
