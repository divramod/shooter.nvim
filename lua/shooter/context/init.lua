...
-- Check if a filepath is a shotfile (in .shooter/project-shotfiles folder)
function M.is_shotfile(filepath)
  if not filepath or filepath == '' then return false end
  return filepath:match('/.shooter/project%-shotfiles/') ~= nil
    or filepath:match('/.shooter/project%-shotfiles$') ~= nil
end
