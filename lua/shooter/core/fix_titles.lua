...
-- Canonical rule:
--   .hal/shooter/shotfiles/foo.md            → "# foo"
--   .hal/shooter/shotfiles/some/test/foo.md  → "# some/test/foo"
...
-- Collect all shotfile paths under .hal/shooter/shotfiles for the repo.
-- Also includes projects/*/.hal/shooter/shotfiles for mono-repos.
function M.collect_shotfiles(git_root)
  git_root = git_root or files.get_git_root()
  if not git_root then return {} end

  local dirs = {}
  local root_dir = git_root .. '/.hal/shooter/shotfiles'
  ...
  if utils.dir_exists(projects_dir) then
    for _, name in ipairs(vim.fn.readdir(projects_dir)) do
      local proj_dir = projects_dir .. '/' .. name .. '/.hal/shooter/shotfiles'
