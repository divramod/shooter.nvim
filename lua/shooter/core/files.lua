...
function M.is_in_prompts_folder(path)
  ...
  if git_root then
    if path:find(git_root .. '/.shooter/ai/shotfiles', 1, true) then
      return true
    end
    if path:find(git_root .. '/projects/.+/.shooter/ai/shotfiles') then
      return true
    end
  end
  return false
...
-- Get shooter files from directory (returns display paths without .shooter/ai/shotfiles prefix)
-- project = nil means root level, string means specific project
function M.get_prompt_files(project)
  ...
  for _, file in ipairs(files) do
    -- Store both display path (without .shooter/ai/shotfiles/) and full path
