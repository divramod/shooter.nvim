...
local function get_prompt_files()
  ...
  local prompts_dir = cwd .. '/docs/shotfiles'
...
function M.move_file(pb, tfolder)
  ...
  local tdir = tfolder == '' and cwd .. '/docs/shotfiles' or cwd .. '/docs/shotfiles/' .. tfolder
  local tpath = tdir .. '/' .. fname
  local dname = tfolder == '' and 'prompts' or tfolder

  vim.fn.mkdir(tdir, 'p')
  if tfolder == '' and vim.fn.fnamemodify(src, ':h') == cwd .. '/docs/shotfiles' then
