...
function M.get_repo_prompt_files()
  ...
  local prompts_dir = git_root .. '/.shooter/project-shotfiles'
...
-- Get files for telescope picker (returns display paths without .shooter/project-shotfiles prefix)
-- opts table supports:
...
function M.get_prompt_files(folder_filter_or_opts, project)
  ...
  if opts.include_all_projects then
    ...
    -- Add root prompts
    add_from_prompts_dir(git_root .. '/.shooter/project-shotfiles', '', nil)
    -- Add all project prompts
    local projects = project_mod.list_projects()
    for _, p in ipairs(projects) do
      add_from_prompts_dir(p.path .. '/.shooter/project-shotfiles', p.name .. '/', p.name)
    end
...
function M.get_all_repos_prompt_files(folder_filter_or_opts)
  ...
  local function add_repo_files(repo_path, repo_name)
    -- Add root prompts
    add_prompts_dir(repo_path .. '/.shooter/project-shotfiles', repo_name .. '/', repo_name)
