...
Every `sho-` command and agent must display a banner as its first output. The current shooter version is **`0.2.55`**.

**Format:** `**\`sho-<name>\`** · **shooter v0.2.55** · model: **<model>**`
...
The `# context` section at the bottom of each shot includes the theme (e.g., "shooter ai", "shooter cli"). Theme mapping is also in `.shooter/themes.yaml`. `_project` is the default meta-theme for cross-cutting work. Read `themes.yaml` if the shot context is ambiguous.
...
| Type | Purpose | Parent |
|---|---|---|
| `theme` | Top-level grouping (project-specific, defined in `.shooter/themes.yaml`) | None |
