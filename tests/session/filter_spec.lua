...
describe('session.filter', function()
  describe('detect_file_folder', function()
    it('should detect archive folder', function()
      local folder = filter.detect_file_folder('/repo/docs/shotfiles/archive/file.md')
      assert.equals('archive', folder)
    end)

    it('should detect backlog folder', function()
      local folder = filter.detect_file_folder('/repo/docs/shotfiles/backlog/task.md')
      assert.equals('backlog', folder)
    end)

    it('should detect prompts (root) for direct files', function()
      local folder = filter.detect_file_folder('/repo/docs/shotfiles/feature.md')
      assert.equals('prompts', folder)
    end)

    it('should handle nested paths', function()
      local folder = filter.detect_file_folder('/repo/docs/shotfiles/done/2024/file.md')
      assert.equals('done', folder)
    end)

    it('should handle project paths', function()
      local folder = filter.detect_file_folder('/repo/projects/myapp/docs/shotfiles/archive/old.md')
      assert.equals('archive', folder)
    end)
  end)
  ...
  describe('detect_file_project', function()
    ...
    it('should return nil for root project files', function()
      local project = filter.detect_file_project(git_root .. '/docs/shotfiles/file.md', git_root)
      assert.is_nil(project)
    end)

    it('should detect subproject name', function()
      local project = filter.detect_file_project(git_root .. '/projects/myapp/docs/shotfiles/file.md', git_root)
  ...
  describe('apply_folder_filter', function()
    local files = {
      { path = '/repo/docs/shotfiles/file.md', display = 'file.md' },
      { path = '/repo/docs/shotfiles/archive/old.md', display = 'archive/old.md' },
      { path = '/repo/docs/shotfiles/backlog/todo.md', display = 'backlog/todo.md' },
    }
  ...
  describe('apply_project_filter', function()
    local git_root = '/repo'
    local files = {
      { path = '/repo/docs/shotfiles/root.md' },
      { path = '/repo/projects/app-a/docs/shotfiles/a.md' },
      { path = '/repo/projects/app-b/docs/shotfiles/b.md' },
    }
