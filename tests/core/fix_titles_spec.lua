...
describe('shooter.core.fix_titles', function()
  ...
  describe('files.title_from_path', function()
    it('returns filename only for root shotfiles', function()
      assert.equals('feats', files.title_from_path('/repo/.hal/shooter/shotfiles/feats.md'))
    end)

    it('returns path/name for subfolder shotfiles', function()
      assert.equals('some/test/shotfile',
        files.title_from_path('/repo/.hal/shooter/shotfiles/some/test/shotfile.md'))
    end)

    it('handles project shotfiles (projects/X/.hal/shooter/shotfiles)', function()
      assert.equals('foo/bar',
        files.title_from_path('/repo/projects/myproj/.hal/shooter/shotfiles/foo/bar.md'))
    end)

    it('strips .md extension', function()
      local title = files.title_from_path('/repo/.hal/shooter/shotfiles/a/b.md')
  ...
  describe('collect_shotfiles', function()
    ...
    it('includes project shotfiles dirs', function()
      ...
      local proj_dir = test_root .. '/projects/myproj/.hal/shooter/shotfiles'
  ...
  describe('build_commit_message', function()
    it('uses a singular subject for one change', function()
      local msg = fix_titles.build_commit_message('/repo', {
        { path = '/repo/.hal/shooter/shotfiles/foo.md', old = 'wrong', new = 'foo' },
      })
      assert.truthy(msg:find('canonicalize 1 H1 title\n', 1, true))
      assert.truthy(msg:find('- .hal/shooter/shotfiles/foo.md: "wrong" %-> "foo"'))
    end)

    it('uses a plural subject for multiple changes', function()
      local msg = fix_titles.build_commit_message('/repo', {
        { path = '/repo/.hal/shooter/shotfiles/a.md', old = 'a0', new = 'a' },
        { path = '/repo/.hal/shooter/shotfiles/b/c.md', old = 'c', new = 'b/c' },
      })
      assert.truthy(msg:find('canonicalize 2 H1 titles'))
      assert.truthy(msg:find('- .hal/shooter/shotfiles/a.md:'))
      assert.truthy(msg:find('- .hal/shooter/shotfiles/b/c.md: "c" %-> "b/c"'))
    end)

    it('labels missing old titles', function()
      local msg = fix_titles.build_commit_message('/repo', {
        { path = '/repo/.hal/shooter/shotfiles/fresh.md', old = nil, new = 'fresh' },
      })
  ...
  describe('commit_fixes', function()
    ...
    local shotfiles = repo .. '/.hal/shooter/shotfiles'
    ...
    it('commits a fixed shotfile with descriptive message', function()
      ...
      assert.truthy(log:find('- .hal/shooter/shotfiles/foo.md: "wrong" %-> "foo"'))
    end)
    ...
    it('reports error if git_root is not a git repo', function()
      ...
      utils.write_file(not_repo .. '/.hal/shooter/shotfiles/foo.md', '# foo\n')

      local ok, err = fix_titles.commit_fixes(not_repo, {
        { path = not_repo .. '/.hal/shooter/shotfiles/foo.md', old = 'x', new = 'foo' },
      })
      assert.is_false(ok)
      assert.truthy(err)
