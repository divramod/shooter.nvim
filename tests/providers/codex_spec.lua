-- Tests for shooter.providers.codex file-send behavior
...
describe('providers.codex', function()
  ...
  before_each(function()
    package.loaded['shooter.providers.codex'] = nil
    codex = require('shooter.providers.codex')
  ...
  it('returns error when pane id is missing', function()
    local ok, err = codex.send_file_reference(nil, '/tmp/shot.md')
    assert.is_false(ok)
    assert.are.equal('No pane ID provided', err)
  end)

  it('returns error when filepath is missing', function()
    local ok, err = codex.send_file_reference('%1', nil)
    assert.is_false(ok)
  ...
  it('sends literal filepath without @ prefix', function()
    ...
    local ok, err = codex.send_file_reference('%1', '/outside/shot-1.md')
