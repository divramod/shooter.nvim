...
describe('providers.gemini', function()
  ...
  it('sends cat command via tmux for valid file', function()
    local ok, err = gemini.send_file_reference('%1', '/tmp/shot.md')
    assert.is_true(ok)
    assert.is_nil(err)
  end)

  it('sends cat command via tmux for missing file', function()
    -- Gemini provider sends a tmux command (doesn't read file itself)
