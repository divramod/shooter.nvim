...
1. **Create a new shots file**:
   ...
   :ShoCreate
   ```
   Enter a feature name when prompted. This creates a timestamped markdown file in `docs/shotfiles/`.
...
require('shooter').setup({
  ...
  paths = {
    ...
    queue_file = 'docs/shotfiles/.shot-queue.json',
...
| `{{file_path}}` | File path (with ~ for home) | `~/dev/docs/shotfiles/file.md` |
...
```
repo/
├── docs/shotfiles/              # Root-level prompts
│   └── shared-feature.md
└── projects/
    ├── frontend/
    │   └── docs/shotfiles/      # Frontend project prompts
    │       └── add-login.md
    └── backend/
        └── docs/shotfiles/      # Backend project prompts
            └── api-routes.md
```
...
3. **Root option**: The picker includes "(root)" to create files at `docs/shotfiles/` instead
...
Repos without a `projects/` folder work exactly as before - all commands create/list files at `docs/shotfiles/`.
